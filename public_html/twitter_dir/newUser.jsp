...
<%
	...
	String query = "select user_id from egoldman.user_t where username = '" + email +"'";
	...
	java.sql.PreparedStatement ps = conn.prepareStatement("insert into egoldman.user_t (name,username,password) values(?,?,?)");
	...
	if(status > 0)
	{
		...
		java.sql.ResultSet rs3 = stmt1.executeQuery("select user_id from user_t where username = '" + email + "'");
		int p_key = 0;
		while(rs3.next())
		{
